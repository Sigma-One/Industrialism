...
class ItemWireSpool(settings: Settings?, val maxLength: Int, val isInfinite: Boolean = false) : Item(settings) {
        ...
        if (!context.world.isClient) {
            if (context.world.getBlockEntity(context.blockPos) is BlockEntityWireNode) {
                ...
                if (this !in (context.world.getBlockEntity(context.blockPos) as BlockEntityWireNode).wireTypes) {
                    context.player!!.sendMessage(TranslatableText("popup.industrialism.wire.failed.type"), true)
                    return ActionResult.PASS
                }

                if (linkA == null) {
                    linkA = context.world.getBlockEntity(context.blockPos)
                    context.player!!.sendMessage(TranslatableText("popup.industrialism.wire.source", "(" + linkA!!.pos.toShortString() + ")"), true)
                    return ActionResult.SUCCESS
                }

                else if (linkA!!.pos === context.blockPos) {
                    context.player!!.sendMessage(TranslatableText("popup.industrialism.wire.failed.self"), true)
                    linkA = null
                    return ActionResult.PASS
                    ...
                    if (!(linkA as BlockEntityWireNode?)!!.connections.contains(context.blockPos) && !(context.world.getBlockEntity(context.blockPos) as BlockEntityWireNode?)!!.connections.contains((linkA as BlockEntityWireNode).pos)) {
                        ...
                        context.player!!.sendMessage(TranslatableText("popup.industrialism.wire.linked", "(" + (linkA as BlockEntityWireNode).pos.toShortString() + ")", "(" + context.blockPos.toShortString() + ")"), true)
                        linkA = null
